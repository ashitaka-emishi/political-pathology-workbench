...
import { validateScoreIndependence } from "../src-js/validate/validate-score-semantics.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const fixturesDir = path.join(__dirname, "fixtures", "score-independence");
...
for (const fileName of fs.readdirSync(fixturesDir).sort()) {
  ...
  const errors = [];
  const warnings = [];
  validateScoreIndependence(fixture.score, errors, warnings, fixturePath, Boolean(fixture.publicFacing));
...
console.log("All score-independence fixtures validated as expected.");
