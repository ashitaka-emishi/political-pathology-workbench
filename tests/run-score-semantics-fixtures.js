...
import { validateScoreSemantics } from "../src-js/validate/validate-score-semantics.js";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const fixturesDir = path.join(__dirname, "fixtures", "score-semantics");
...
for (const fileName of fs.readdirSync(fixturesDir).sort()) {
  ...
  const score = JSON.parse(fs.readFileSync(fixturePath, "utf8"));
  const errors = [];
  const warnings = [];
  validateScoreSemantics(score, errors, warnings, fixturePath);
...
console.log("All score-semantics fixtures validated as expected.");
