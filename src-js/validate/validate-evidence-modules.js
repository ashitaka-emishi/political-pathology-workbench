...
export function validateModuleRecord(moduleRecord, moduleIds, caseIds, errors, label) {
  try {
  ...
  for (const caseId of moduleRecord.caseIds ?? []) {
    if (caseIds.size > 0 && !caseIds.has(caseId)) errors.push(`${label}: unknown caseId ${caseId}`);
  }
}

export function validateEvidenceModules(root, caseIds = new Set()) {
  const errors = [];
  const warnings = [];
  ...
  for (const moduleRecord of modules) {
    ...
    validateModuleRecord(moduleRecord, moduleIds, caseIds, errors, label);
  }
